...
这是一个基于 Fresh 框架的反向代理中间件项目，提供 HTTP/HTTPS 代理服务。项目使用
Deno 运行时，并支持自定义 token 认证。
...
1. 通过 URL 路径格式 `/token/{token}/http/{target_url}` 或
   `/token/{token}/https/{target_url}` 指定代理目标
...
项目包含一个示例 API 端点
`routes/api/joke.ts`，用于返回随机笑话，可用于测试服务器是否正常运行。
